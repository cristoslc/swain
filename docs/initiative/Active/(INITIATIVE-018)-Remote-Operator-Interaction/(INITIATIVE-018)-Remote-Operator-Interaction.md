...
### Chat Bridge (v1)
Watchdog (process manager, EPIC-084) ensures project bridges are running. Each ProjectBridge (EPIC-071) is a self-contained microkernel that spawns its own chat adapter and runtime adapter subprocesses. No hub routing (ADR-046).
...
- EPIC-084 (Watchdog Process Manager) — process lifecycle, ensures project bridges are running.
...
- EPIC-085 (Worktree Discovery) — continuous polling for worktree sessions.
...
- ADR-046 (Project-Level Microkernel) — project bridge as self-contained microkernel.
- ADR-047 (Watchdog Process Manager) — watchdog manages bridge lifecycle.
...
| Active | 2026-04-18 | -- | Architecture refactor: hub-and-spoke replaced by project-level microkernel (ADR-046, ADR-047). EPIC-070 replaced by EPIC-084, added EPIC-085. |
