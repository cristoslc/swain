...
During the ADR-023 migration (deprecating swain-session), we discovered that 30+ cross-skill references needed updating. The BDD test suite written for ADR-023 (66 assertions) caught these effectively using grep-based structural checks. This pattern should be generalized into a standing test suite that runs on every commit touching skill files.
