...
- **Input:** Control thread message: `/swain-do dispatch SPEC-123`
- **Preconditions:**
...
- **Given** the operator messages `/swain-do dispatch SPEC-123` in the control thread, **When** the project bridge processes the command, **Then** swain-do is invoked and a worktree is created.
...
- **Given** an existing session bound to SPEC-123, **When** `/swain-do dispatch SPEC-123` is received, **Then** the project bridge responds with a warning and offers to reconnect to the existing thread.
- **Given** the `/swain-do dispatch` command specifies a non-existent artifact, **When** the project bridge resolves it, **Then** it responds with "Artifact not found" and lists suggestions.
...
**In scope:**
- Control thread command parsing for `/swain-do dispatch <artifact-id>`.
...
1. **Extend control thread handler** — parse `/swain-do dispatch <artifact-id>` and forward to project bridge.
...
- **Cycle 1:** `/swain-do dispatch` command parsing and artifact resolution.
