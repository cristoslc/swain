...
- **SPIKE-048**: Evaluated Backlog.md as a replacement — dependency tracking CLI commands are missing (ready/blocked/dep), blocking adoption
...
Blocked on missing `ready`/`blocked`/`dep` CLI commands since SPIKE-048. The internal engine supports it but the CLI doesn't expose it. Contributing upstream (~180 LOC) is possible but depends on maintainer acceptance timeline.
...
|-------|------|--------|-------|
| Adopted | 2026-03-12 | — | Decision made based on SPIKE-048, -002, -005, -006 findings |
