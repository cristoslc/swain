...
The landscape of projects that bridge agentic coding runtimes to web and chat UIs has matured rapidly in early 2026. This synthesis evaluates the build-vs-buy landscape for VISION-006 (Untethered Operator).
...
No project provides a general-purpose "route this approval request to a chat surface and wait for a response" mechanism. (`secure-openclaw-composio`, `cloudcli-claude-code-ui`, `claude-code-telegram-bot`, `acpx-agent-client-protocol`)
...
1. **ACPX** for runtime adaptation (session lifecycle, event normalization, 16+ agents).
2. **Vercel AI SDK ACP provider** for exposing agents as standard LanguageModel instances to web/Node.js apps.
3. **OpenClaw or custom adapter** for messaging platform routing (Telegram, Discord, etc.).

This stack avoids vendor lock-in to any single product and matches the "glue-code over build-from-scratch" priority. (`acpx-agent-client-protocol`, `acp-vercel-ai-sdk-provider`, `openclaw-ai-assistant`)
...
- **ACP maturity:** ACPX self-reports as alpha. Zed's ACP spec is expanding. The casys.ai guide treats it as production-ready. Real-world stability is unclear.
