---
source-id: "001"
title: "cognee meta-skill SKILL.md — Skill Self-Improvement"
type: local
path: "cognee/cognee_skills/meta-skill/SKILL.md"
url: "https://github.com/topoteretes/cognee/blob/demo/graphskill_COG-4178/cognee/cognee_skills/meta-skill/SKILL.md"
...
1. Call `inspect_skill(skill_id)` — returns `failure_category`, `root_cause`, `severity`, `improvement_hypothesis`.
2. Review the diagnosis — does the root cause match what you've observed?
3. Call `preview_amendify_skill(skill_id)` — returns `amendment_id`, `change_explanation`, `amended_instructions`, `amendment_confidence`.
...
2. `inspect_skill` needs at least 1 failed run (default) to produce a diagnosis
3. `amendment_confidence` below 0.6 is a signal to use the manual flow
4. Run `evaluate_amendify_skill` after a few post-amendment executions, not immediately
5. `rollback_amendify_skill` restores the original instructions but keeps the amendment and inspection history
6. Each amendment stores the original instructions — rollback is always available
7. `observe_skill_run` is called automatically by `run_skill` and `execute_skill`
