...
Backlog.md's internal `TaskGraph` could theoretically support `ready`/`blocked` queries (see SPIKE-048 findings). However, reimplementing specgraph's full command set (`blocks`, `blocked-by`, `tree`, `overview`, `mermaid`) against Backlog.md's data format would require:
