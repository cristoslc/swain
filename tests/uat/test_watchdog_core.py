"""UAT: SPEC-318 — Watchdog Core (executable integration tests).

Tests process management, PID files, health checks, reconciliation,
graceful shutdown, and edge cases. These tests spawn real watchdog processes.
"""
...
skip_if_not_docker = pytest.mark.skipif(
    not IS_DOCKER,
    reason="Process tests require Docker isolation",
)
...
class TestWatchdogZeroConfigs:
    ...
    def test_watchdog_runs_with_no_projects(
        ...
        watchdog.start()
        assert watchdog.proc is not None, "Watchdog process not started"
...
class TestWatchdogGracefulShutdown:
    ...
    def test_bridges_stopped_on_sigint(
        ...
        watchdog.start()
        pid_file = config_dir / "run" / "bridges" / "alpha.pid"
        bridge_pid = int(pid_file.read_text().strip().split("\n")[0])
        assert watchdog.proc is not None
        watchdog.proc.send_signal(signal.SIGINT)
        try:
            watchdog.proc.wait(timeout=15)
        except subprocess.TimeoutExpired:
            watchdog.proc.kill()
...
class TestWatchdogAlreadyRunning:
    """SPEC-318 NEG-01: second watchdog instance detects existing one."""

    def test_second_instance_exits_gracefully(
        self, config_dir: Path, watchdog: _WatchdogProcess
    ):
        _write_project(config_dir, "alpha")
        watchdog.start()
        ...
        assert proc2.returncode is not None, "Second instance did not exit"


class TestWatchdogBridgeCrashRecovery:
    """SPEC-318 NEG-02: bridge that crashes is restarted on next cycle."""

    def test_crashed_bridge_restarted(
        self, config_dir: Path, watchdog: _WatchdogProcess
    ):
        _write_project(config_dir, "alpha")
        watchdog.start()
        pid_file = config_dir / "run" / "bridges" / "alpha.pid"
        old_pid = int(pid_file.read_text().strip().split("\n")[0])
        os.kill(old_pid, signal.SIGKILL)
        time.sleep(2)
        time.sleep(35)
        if pid_file.exists():
            new_pid = int(pid_file.read_text().strip().split("\n")[0])
            if new_pid > 0:
                try:
                    os.kill(new_pid, 0)
...
class TestWatchdogConfigDirCreation:
    """SPEC-318 NEG-03: watchdog creates config dir on startup if missing."""

    def test_creates_missing_config_dirs(self, tmp_path: Path):
        cfg = _make_config(tmp_path)
        _write_project(cfg, "alpha")
        wd = _WatchdogProcess(cfg)
        try:
            wd.start()
            assert (wd.config_dir / "run").exists()
        finally:
            wd.stop()
