...
_HOST_EVENT_TYPES = {
    "host_status", "unmanaged_session_found", "unmanaged_session_gone",
...
class Event:
    ...
    def text_output(cls, *, bridge: str, session_id: str, content: str) -> Event:
        return cls(
            type="text_output", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(), payload={"content": content},
        )

    @classmethod
    def tool_call(cls, *, bridge: str, session_id: str, tool_name: str,
                  input: dict[str, Any], call_id: str) -> Event:
        return cls(
            type="tool_call", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(),
            payload={"tool_name": tool_name, "input": input, "call_id": call_id},
        )

    @classmethod
    def tool_result(cls, *, bridge: str, session_id: str, call_id: str,
                    output: str, success: bool) -> Event:
        return cls(
            type="tool_result", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(),
            payload={"call_id": call_id, "output": output, "success": success},
        )

    @classmethod
    def approval_needed(cls, *, bridge: str, session_id: str, tool_name: str,
                        description: str, call_id: str) -> Event:
        return cls(
            type="approval_needed", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(),
            payload={"tool_name": tool_name, "description": description, "call_id": call_id},
        )

    @classmethod
    def session_spawned(cls, *, bridge: str, session_id: str, runtime: str,
                        artifact: str | None = None) -> Event:
    ...
    def session_died(cls, *, bridge: str, session_id: str, reason: str) -> Event:
        return cls(
            type="session_died", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(), payload={"reason": reason},
        )

    @classmethod
    def web_output_available(cls, *, bridge: str, session_id: str,
                             path_or_port: str, label: str) -> Event:
        return cls(
            type="web_output_available", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(),
    ...
    def host_status(cls, *, bridges_running: int, disk: str, load: str) -> Event:
        return cls(
            type="host_status", bridge="__host__", session_id=None,
            timestamp=_now_ms(),
            payload={"bridges_running": bridges_running, "disk": disk, "load": load},
        )

    @classmethod
    def unmanaged_session_found(cls, *, tmux_target: str,
                                 runtime_hint: str | None = None,
                                 project_path: str | None = None) -> Event:
        ...
        return cls(
            type="unmanaged_session_found", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload=payload,
        )
    ...
    def unmanaged_session_gone(cls, *, tmux_target: str) -> Event:
        return cls(
            type="unmanaged_session_gone", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload={"tmux_target": tmux_target},
        )

    @classmethod
    def bridge_started(cls, *, project: str, bridge_id: str) -> Event:
        return cls(
            type="bridge_started", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload={"project": project, "bridge_id": bridge_id},
        )

    @classmethod
    def bridge_stopped(cls, *, project: str, bridge_id: str, reason: str) -> Event:
        return cls(
            type="bridge_stopped", bridge="__host__", session_id=None,
            timestamp=_now_ms(),
...
class Command:
    ...
    @classmethod
    def start_session(cls, *, bridge: str, runtime: str,
                      artifact: str | None = None,
                      prompt: str | None = None) -> Command:
        payload: dict[str, Any] = {"runtime": runtime}
        if artifact:
            payload["artifact"] = artifact
        if prompt:
    ...
    @classmethod
    def approve(cls, *, bridge: str, session_id: str, call_id: str,
                approved: bool) -> Command:
        return cls(
            type="approve", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(),
    ...
    @classmethod
    def bind_artifact(cls, *, bridge: str, session_id: str,
                      artifact_id: str) -> Command:
        return cls(
            type="bind_artifact", bridge=bridge, session_id=session_id,
            timestamp=_now_ms(), payload={"artifact_id": artifact_id},
        )

    # --- Host-scope factory methods ---

    @classmethod
    def clone_project(cls, *, repo_url: str,
                      host_path: str | None = None) -> Command:
        payload: dict[str, Any] = {"repo_url": repo_url}
        if host_path:
            payload["host_path"] = host_path
        return cls(
            type="clone_project", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload=payload,
        )
    ...
    def init_project(cls, *, project_path: str) -> Command:
        return cls(
            type="init_project", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload={"project_path": project_path},
        )

    @classmethod
    def start_bridge(cls, *, project_path: str) -> Command:
        return cls(
            type="start_bridge", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload={"project_path": project_path},
        )
    ...
    def stop_bridge(cls, *, project: str) -> Command:
        return cls(
            type="stop_bridge", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload={"project": project},
        )

    @classmethod
    def adopt_session(cls, *, tmux_target: str, project: str,
                      runtime: str | None = None,
                      artifact: str | None = None) -> Command:
        ...
        if artifact:
            payload["artifact"] = artifact
        return cls(
            type="adopt_session", bridge="__host__", session_id=None,
            timestamp=_now_ms(), payload=payload,
        )
