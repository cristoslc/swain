...
- **Value estimate (optional):** `value-estimate:` in frontmatter. Object with `capability` (0-3), `efficiency` (0-3), `risk-reduction` (0-3). Overrides the parent's value for this SPEC. Omit to inherit from the parent chain.
- **Cost estimate (optional):** `cost-estimate:` in frontmatter. T-shirt size (XS/S/M/L/XL). Combined with value-estimate to compute ROI (composite value / cost). Higher ROI specs rank above lower ROI specs at equal structural leverage and priority weight.
