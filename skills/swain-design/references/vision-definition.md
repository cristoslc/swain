...
- **Value estimate (optional):** `value-estimate:` in frontmatter. An object with three dimensions: `capability` (0-3), `efficiency` (0-3), `risk-reduction` (0-3). Expresses how much value this vision's work delivers. Cascades to child Initiatives, Epics, and Specs that lack their own estimate. Visions do not carry `cost-estimate` — cost is assessed at the Initiative/Epic/Spec level where scope is concrete enough to estimate.
