...
- **Value estimate (optional):** `value-estimate:` in frontmatter. Object with `capability` (0-3), `efficiency` (0-3), `risk-reduction` (0-3). Overrides the parent Initiative/Vision value for this Epic and its children. When absent, value is inherited from the parent chain.
- **Cost estimate (optional):** `cost-estimate:` in frontmatter. T-shirt size (XS/S/M/L/XL). Together with value-estimate, produces an ROI ratio that feeds into recommendation scoring.
