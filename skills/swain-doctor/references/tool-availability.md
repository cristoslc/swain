...
|------|-------|---------|-------------|---------------------|--------|
| `tk` | `[ -x "$SKILLS_ROOT/swain-do/bin/tk" ]` | swain-do, swain-session (tasks) | Task tracking unavailable; status skips task section | Vendored at `swain-do/bin/tk` -- reinstall swain if missing | report-only |
| `uv` | `command -v uv` | swain-do (plan ingestion) | Plan ingestion unavailable | `brew install uv` | bundle-offer |
| `gh` | `command -v gh` | swain-session (GitHub issues), swain-release | Status skips issues section; release can't create GitHub releases | `brew install gh` | bundle-offer |
| `tmux` | `which tmux` | swain-session | Session tab-naming unavailable outside tmux | `brew install tmux` | bundle-offer |
