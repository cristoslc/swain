...
1. Use the `EnterWorktree` tool to create an isolated worktree. **Always pass a unique name** — use the SPEC ID + slug (e.g., `spec-174-branch-collision`) or a timestamped name (e.g., `session-20260327-143022`). Never use a static name like "session" — concurrent sessions will collide (SPEC-174). This is the only mechanism that actually changes the agent's working directory — manual `git worktree add` + `cd` does not persist across tool calls.
