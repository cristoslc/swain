...
- **Tracking requirement:** All Bugs carry `execution-tracking: required` in frontmatter. When a Bug comes up for implementation (typically at Reported → Active), invoke the execution-tracking skill to create a tracked plan before writing the fix. The first task in the plan should be a **failing regression test** that reproduces the bug — this anchors the fix to observable behavior and prevents regressions. The Bug's `fix-ref` field records the execution-tracking plan or task ID (see SKILL.md § Execution tracking handoff).
