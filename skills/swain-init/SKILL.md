...
- **Same major version** (e.g., both `4.x.x`): Project is current. Tell the user:
  > Project already initialized (swain $LAST_VERSION). Starting session.
...
If the user runs `/swain-init` on a project that's already set up, Phase 0 reads `.swain-init` and skips to Phase 7 (Session Start) — no onboarding phases run, no interactive prompts appear. This lets users build muscle memory around `/swain-init` as a single entry point.
